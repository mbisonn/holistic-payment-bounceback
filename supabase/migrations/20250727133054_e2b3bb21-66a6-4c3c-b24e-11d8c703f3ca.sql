-- Create upsell_transactions table for upsell payment tracking
CREATE TABLE IF NOT EXISTS public.upsell_transactions (
  id UUID NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
  product_id UUID REFERENCES public.upsell_products(id) ON DELETE CASCADE,
...
DROP POLICY IF EXISTS "Admin only access to upsell transactions" ON public.upsell_transactions;
CREATE POLICY "Admin only access to upsell transactions" ON public.upsell_transactions FOR ALL USING (current_user_is_admin());
...
CREATE INDEX IF NOT EXISTS idx_upsell_transactions_email ON public.upsell_transactions(customer_email);
